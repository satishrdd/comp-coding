...
void dfs(int a,int b,int source){
  visited[a][b] = true;
 rep(i,0,4){
  int px = a + mx[i],py = b+my[i];

  if(px<0||px>=n||py<0||py>=m||c[a][b]!=c[px][py])
    continue;
  if(visited[px][py]&&i!=source) f=1;
  if(!visited[px][py]) dfs(px,py,(i+2)%4);
 }
} 
