...
int main(int argc, char const *argv[])
{
	...
	long long ar[n],m[n];
	if(n<=0){
		cout<<"Case "<<(l-cases)<<": 0"<<endl;
	}else if(n==1){
		cin>>ar[0];
		cout<<"Case "<<(l-cases)<<": "<<ar[0]<<endl;
	}else if (n==2){
		cin>>ar[0]>>ar[1];
		cout<<"Case "<<(l-cases)<<": "<<max(ar[0],ar[1])<<endl;
	}else{
		cin>>ar[0]>>ar[1];
		m[0]=ar[0];
		m[1] = max(ar[0],ar[1]);
		for(int i=2;i<n;i++){
			cin>>ar[i];
			if(i>=3)
			m[i] = max(m[i-2]+ar[i],max(ar[i]+m[i-3],m[i-1]));
			else{
				m[i] = max(m[i-2]+ar[i],m[i-1]);	
			}
		}
		cout<<"Case "<<(l-cases)<<": "<<m[n-1]<<endl;
	}}
	return 0;
}